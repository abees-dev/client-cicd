...
export default function NotistackProvider({ children }: IPropsTypes<ReactNode>) {
  return (
    <>
      <SnackBarStyle />
      <SnackbarProvider
...
function SnackbarIcon({ icon, color }: ISnackbarIcon) {
  return (
    <>
      <Box
        sx={{ bgcolor: (theme: Theme) => alpha(theme.palette[color].main, 0.1), position: 'absolute', inset: 0 }}
      ></Box>
      <Box
        component="span"
        sx={{
